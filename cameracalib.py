...
workingFolder   = "./Calibration Images"
imageType       = 'jpg'
#------------------------------------------
...
if len(images) < 9:
    ...
    for fname in images:
        ...
        gray    = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)#a grayscale image is required for calibration. 

        print("Reading image ", fname)

        # Find the chess board corners
        ret, corners = cv2.findChessboardCorners(gray, (nCols,nRows),None)#changed gray to img
        fordrawing = cv2.findChessboardCorners(gray, (nCols,nRows),None)
        # If found, add object points, image points (after refining them)
        if ret == True:
            print("Pattern found! Press ESC to skip or ENTER to accept")
            #--- Sometimes, Harris cornes fails with crappy pictures, so
            corners2 = cv2.cornerSubPix(gray,corners,(11,11),(-1,-1),criteria)#changed gray to img

            # Draw and display the corners
            cv2.drawChessboardCorners(img, (nCols,nRows), corners2,ret)#changed img to resized to display smaller image
            cv2.imshow('img',img)#changed img to resized to display smaller image
            # cv2.waitKey(0)
...
if (nPatternFound > 1):
    ...
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1],None,None)#changed gray to img
